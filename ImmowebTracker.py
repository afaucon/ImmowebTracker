...
def get_list_of_id(url):
    ...
    with open("C:\\Data\\temp5\\" + filename, "wb") as myfile:
        myfile.write(html_doc)

    filename = time.strftime("%Y%m%d-%H%M%S_soup2") + ".log"
    with open("C:\\Data\\temp5\\" + filename, "w", encoding='utf-8') as myfile:
...
def get_new_real_estate_ad():
    ...
    file_path = "C:\\Data\\temp5\\list_of_id.txt"
...
if __name__ == '__main__':
    ...
    if debug == True:
        ...
        if tests[selection] == send_to_pushbullet:
            new_list_of_id = [1234567,6666666]
